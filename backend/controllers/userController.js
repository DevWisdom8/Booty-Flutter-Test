...
const buildVerificationLink = (token) => {
  const baseUrl =
    ...
    `${
      (process.env.APP_BASE_URL || "http://localhost:5004").replace(/\/$/, "")
    }/api/users/verify-email`;

  if (!baseUrl) {
    throw new Error("EMAIL_VERIFICATION_URL or APP_BASE_URL must be configured");
  }
...
exports.registerUser = asyncHandler(async (req, res, next) => {
  ...
  console.log("registerUser called with", { email, username });
  let newUserObject = {};
  let credentials;
  ...
  console.log("Existing user check:", userExists ? "found" : "not found", { email });
  ...
  let firebaseUserAlreadyExists = false;
  try {
    ...
    if (error.code === "auth/email-already-in-use") {
      firebaseUserAlreadyExists = true;
  ...
  const configuredTTL = Number(process.env.EMAIL_VERIFICATION_TTL_HOURS);
  const verificationTTL = Number.isFinite(configuredTTL) && configuredTTL > 0 ? configuredTTL : 24;
  user.emailVerificationToken = hashedVerificationToken;
  user.emailVerificationExpires = new Date(Date.now() + verificationTTL * HOURS_TO_MS);

  try {
    await user.save();
    console.log("Saved user document", { id: user._id, email: user.email });
  } catch (error) {
    console.log(error);
    res.status(500);
    throw new Error("Unable to persist verification token");
  }
  ...
  if (user) {
    ...
    console.error("Invalid user data during registerUser", { email, username });
    res.status(500);
...
exports.verifyEmail = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    const failureRedirect = process.env.EMAIL_VERIFICATION_FAILURE_REDIRECT;
    if (failureRedirect) {
      return res.redirect(failureRedirect);
    }
    return res
      .status(400)
      .send("<h1>Verification failed</h1><p>This verification link is invalid or has expired.</p>");
  }
  ...
  const successRedirect = process.env.EMAIL_VERIFICATION_SUCCESS_REDIRECT;
  if (successRedirect) {
  ...
  return res
    .status(200)
    .send("<h1>Email verified</h1><p>Your email address has been successfully verified. You may now return to the app.</p>");
});
